...
pub fn build_vote_commitment(
    ...
    // the proof is built inline on the calling worker.
    let build_vote_proof = move || {
    ...
    let vote_bundle = build_vote_proof().map_err(|e| VotingError::ProofFailed {
        message: format!("vote proof generation failed: {}", e),
    })?;
