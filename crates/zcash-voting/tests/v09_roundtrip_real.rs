...
//!
//! All crypto is real. The Merkle auth-path fed to ZKP #2 is a placeholder
//! because the chain does NOT verify the delegation/vote ZK proofs or path
//! membership (see x/vote/keeper/msg_server.go): the only crypto the chain
//! actually exercises is the ElGamal vote_commitment + the 16 encrypted shares,
//! which the helper decrypts at tally. Those are produced for real here.
//!
...
fn v09_roundtrip_real() {
    ...
    // The auth-path is a placeholder: the chain does not verify the vote
    // proof or path membership; the vote_commitment + shares are real.
    // ---------------------------------------------------------------
    let auth_path: Vec<[u8; 32]> = vec![[0u8; 32]; 24];
