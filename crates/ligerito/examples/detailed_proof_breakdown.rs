...
use ligerito::{hardcoded_config_20, prove_sha256};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        initial_proof_size,
        initial_proof_size as f64 / 1024.0
    );
    println!(
        "  opened_rows: {} rows",
    ...
    if !proof.initial_ligero_proof.opened_rows.is_empty() {
        let row_size =
            std::mem::size_of::<BinaryElem32>() * proof.initial_ligero_proof.opened_rows[0].len();
        let total_rows_size = row_size * proof.initial_ligero_proof.opened_rows.len();
        println!(
            "    - row size: {} bytes ({} elements × {} bytes)",
            row_size,
            proof.initial_ligero_proof.opened_rows[0].len(),
            std::mem::size_of::<BinaryElem32>()
        );
        println!(
            "    - total rows: {} bytes ({:.2} KB)",
            total_rows_size,
            total_rows_size as f64 / 1024.0
        );
    ...
    println!(
        "  total size: {} bytes ({:.2} KB)",
        final_proof_size,
        final_proof_size as f64 / 1024.0
    );
    ...
    if !proof.final_ligero_proof.opened_rows.is_empty() {
        let row_size =
            std::mem::size_of::<BinaryElem128>() * proof.final_ligero_proof.opened_rows[0].len();
    ...
    let opened_rows_total =
        proof.initial_ligero_proof.opened_rows.len() + proof.final_ligero_proof.opened_rows.len();

    println!("merkle proofs:");
    println!("  total siblings: {}", merkle_total);
    println!(
        "  total size: {} bytes ({:.2} KB) - {:.1}% of proof",
        merkle_total_bytes,
    ...
    println!(
        "  estimated size: {:.2} KB - {:.1}% of proof",
        (initial_proof_size + final_proof_size - merkle_total_bytes) as f64 / 1024.0,
        (initial_proof_size + final_proof_size - merkle_total_bytes) as f64
            / full_serialized.len() as f64
            * 100.0
    );
    ...
    println!(
        "difference:     {:.2} KB ({:.1}% larger)\n",
        difference / 1024.0,
        (difference / ashutosh_size) * 100.0
    );
    ...
    println!(
        "   - our {} siblings could deduplicate to ~50-60%",
        merkle_total
    );
    println!(
        "   - potential savings: ~{:.2} KB\n",
    ...
    println!(
        "   - we have {} Vec allocations in the proof structure",
    ...
    println!(
        "  - {} siblings × 32 bytes = {:.2} KB ({}% of total)",
        merkle_total,
        merkle_total_bytes as f64 / 1024.0,
    ...
    println!(
        "  - deduplicate ~40% of merkle siblings (-{:.2} KB)",
        merkle_total_bytes as f64 * 0.4 / 1024.0
    );
    println!(
        "  - or use compression (16% reduction = -{:.2} KB)",
        difference * 0.16 / 1024.0
    );
