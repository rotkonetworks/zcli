...
fn test_bridge_nested_3of5_inner() {
    use frost_spend::nested::{
        aggregate_inner_commitment_pair, aggregate_validator_shares_verified, inner_precommit,
        validator_commit, validator_sign_v2, verify_inner_precommit, InnerSigningParamsV2,
    ...
    let dealer_a = dkg::Dealer::<Point>::new(1, outer_t, &mut rng).unwrap();
    ...
    let fa_at_p = dealer_a.generate_subshare(nested_position).unwrap();
    ...
    for k in 0..inner_n as usize {
        ...
        validator_shares.push(SecretShare::new((k + 1) as u32, sigma).unwrap());
    }

    // position A's outer secret
    let s1 = *dealer_a.generate_subshare(1).unwrap().value() + fp_at_1;
    let position_a_share = SecretShare::new(1, s1).unwrap();
    ...
    for &k in &active_validators {
        let (nonces, commits) = validator_commit(k, SESSION);
    ...
    // W-1/N-1: every validator derives the outer context itself from the
    // package and the group key. There is no way to hand it loose scalars.
    let params =
...
fn test_bridge_nested_different_subsets() {
    ...
    for subset in subsets {
        ...
        let params =
            InnerSigningParamsV2::from_outer::<Point>(&package, &group_key, nested_position)
                .unwrap();
